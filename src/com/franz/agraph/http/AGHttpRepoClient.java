...
public class AGHttpRepoClient implements Closeable {
     ...
     * @throws AGHttpException
     */
    public void begin() throws AGHttpException {
            String url = getRoot() + "/" + AGProtocol.BEGIN;
            Header[] headers = {};
            getHTTPClient().post(url, headers, new NameValuePair[0],
                                    (RequestEntity) null, null);
    }
	 ...
	 * @throws AGHttpException
	 */
	public boolean isActive() throws AGHttpException {
		String url = getSessionURL(getRoot())+"/isActive";	
		Header[] headers = {};		
		AGStringHandler handler = new AGStringHandler();
		try {
			getHTTPClient().get(url, headers, new NameValuePair[0], handler);
			...
			if (!e.getLocalizedMessage().equals("Not found.")) {
				throw e;
			}
			else {
				throw new AGHttpException("Please upgrade to AllegroGraph-4.14 to access this api");
			}
		}
		return Boolean.parseBoolean(handler.getResult());
	}
